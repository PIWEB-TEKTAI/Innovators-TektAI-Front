...
const TeamDetails = () => {
  ...
  const generateInvitationLink = async () => {
    try {
      const data = await teamService.inviteByLink(teamId); // Assuming teamId is used to generate the link
      setInvitationLink(data.invitationLink);
      setIsInviteMembersModal(true);
    } catch (error) {
  ...
  return (
    <ConnectedClientLayout>
      {isInviteMembersModal && (
        <InviteMembersForm  invitationLink={invitationLink} onClose={() => setIsInviteMembersModal(false)} team={team} invitationLink={invitationLink} />
      )}
      <div className="bg-white  rounded-lg p-4">
        <div className="z-20 flex flex-col md:flex-row md:h-35 bg-gray-100 text-black text-2xl items-center justify-center font-semibold">
          ...
          <div className="ml-1 text-primary">
            {team.name.split('').map((char: any, index: any) => (
              <span
                key={index}
                className={`animated-text drop-shadow-[0_1px_4px_rgba(60,80,224,1)] animate-pulse ${
                  index === 0 ? 'uppercase' : ''
                }${char === ' ' ? 'capitalize' : ''}`}
        ...
        <div className="flex justify-center items-center my-2">
          ...
          <p className="text-sm text-white font-semibold bg-primary rounded-full py-1 px-2 sm:ml-8">Leader</p>
        </div>
        <div className="flex justify-center">
          <p className="text-lg text-black my-4 font-semibold sm:mr-8">Members: </p>
        </div>
        <div>
      ...
      <div className="bg-white rounded-lg my-6">
        <ul className="p-8 flex cursor-pointer flex-wrap sm:flex-nowrap border-gray-200 border-b py-4">
          <li className="-mb-px mr-1">
            <a
              className={`bg-white inline-block rounded-t py-2 px-4 hover:text-blue-700 text-blue-500 font-semibold ${
                activeTab === 'overview' ? 'bg-blue-100 text-blue-700 border-l border-t border-r' : ''
              }`}
        ...
        <div className="p-8">
          {activeTab === 'joinRequests' && <JoinRequests team={team}></JoinRequests>}
