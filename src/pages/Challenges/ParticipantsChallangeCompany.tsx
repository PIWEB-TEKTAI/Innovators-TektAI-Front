...
const Participations = () => {
  ...
  const handleCancelSolo = async (challengeTitle: string, userId: string) => {
    try {
      await axios.put(`http://localhost:3000/challenge/${challengeTitle}/declineParticipationtitle/${userId}`, { type: 'Request' }, { withCredentials: true });
      // Mettez à jour l'état ou effectuez d'autres actions nécessaires après avoir annulé la demande de participation solo
    } catch (error) {
  ...
  <>
    ...
    <button onClick={() => handleCancelSolo(participant.challengeTitle, participant._id)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Cancel</button>
  </>
)}
