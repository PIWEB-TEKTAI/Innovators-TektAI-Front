...
import DateTimePickerStart from '../../components/Forms/DatePicker/DateTimePickerStart';
...
const EditChallenge = () => {
  ...
  const checkOutput = (value:boolean) => {
    if(value === true){
  ...
  const checkPresentation = (value:boolean) => {
    if(value === true){
  ...
  const checkDataset = (value:boolean) => {
    if(value === true){
  ...
  const checkCodeSource = (value:boolean) => {
    if(value === true){
  ...
  const checkReadMeFile = (value:boolean) => {
    if(value === true){
  ...
  const checkRapport = (value:boolean) => {
    if(value === true){
  ...
  const checkMonetary = (value:string) => {
    if(value !== ''){
      setNumberInputVisible(true);
      setTextInputVisible(false);
      setIsMonetaryChecked(true);
      setIsPrizesChecked(false);
    }else{
      setNumberInputVisible(false);
      setTextInputVisible(true)
      setIsMonetaryChecked(false);
      setIsPrizesChecked(true);
    }
  };


  const checkPrize = (value1:string ,value2:string ) => {
    if((value1 !== '') && (value2 !== '')){
  ...
  const checkRecruitement = (value1:string ,value2:string ) => {
    if((value1 !== '') && (value2 !== '')){
  ...
  const checkFreelance = (value1:string ,value2:string ) => {
    if((value1 !== '') && (value2 !== '')){
  ...
  const checkIntern = (value1:string ,value2:string,value3:string ) => {
  ...
  useEffect(() => {
    const fetchChallenge = async () => {
      try {
        ...
        //prizeee
        setprizeName(data?.prizes.prizeName || '');
        setprizeDescription(data?.prizes.prizeDescription || '');
        //recruitement
        setpositionTitle(data?.recruitement.positionTitle || '');
        setjobDescription(data?.recruitement.jobDescription || '');
        //freelance
        setprojectTitle(data?.freelance.projectTitle || '');
        setprojectDescription(data?.freelance.projectDescription|| '');
        //internship
        ...
        checkCodeSource(data?.bareme.codeSource);
        checkReadMeFile(data?.bareme.readmeFile);
        checkRapport(data?.bareme.rapport);
        checkDemo(data?.bareme.Demo);
        checkMonetary(data?.amount);
        checkPrize(data?.prizes.prizeName , data?.prizes.prizeDescription);
        checkRecruitement(data?.recruitement.positionTitle , data?.recruitement.jobDescription);
        checkFreelance(data?.freelance.projectTitle , data?.freelance.projectDescription);
        checkIntern(data?.internship.internshipTitle , data?.internship.internshipDescription,data?.internship.duration);
       } catch (error) {
