...
const SubmissionCard: React.FC<{ challenge: Challenge }> = ({ challenge }) => (
    <div key={challenge._id} className="max-w-sm bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg mx-4 my-6">
        <div className="p-6">
            <h3 className="text-lg font-semibold text-gray-700 mb-4">Solo Participants: {challenge.participations.soloParticipants}</h3>
            ...
            <div className="flex justify-between items-center">
                <div className="inline-flex rounded-full text-sm font-medium px-4 py-2 mb-4">
                    <p className="text-lg font-semibold text-gray-700 mb-4">Team Requests: {challenge.participations.TeamParticipationRequests}</p>
                </div>
                <div className="inline-flex rounded-full text-sm font-medium px-4 py-2 mb-4">
                    <p className="text-lg font-semibold text-gray-700 mb-4">Solo Requests: {challenge.participations.soloParticipationRequests}</p>
                </div>
            </div>
        </div>
    </div>
...
export default function FetchData() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/challenges/challenges/${id}`);
