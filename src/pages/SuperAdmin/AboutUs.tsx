...
const AboutUs = () => {
  ...
  return (
    <Layout>
      <div className="rounded-sm border border-stroke mb-10 bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      {showAlert && (
              <div className="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] mb-5 px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            <div className="flex justify-end">
              <button
                type="submit"
                className="rounded-sm bg-orange-500 p-2 px-3 text-md font-medium text-gray hover:bg-opacity-90"
              >
                Edit
              </button>
          ...
          {showAlert1 && (
                <div className="flex w-full border-l-6 border-[#34D399] bg-[#34D399] bg-opacity-[15%] mb-3 px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
                  <div className="mr-5 flex h-9 w-full max-w-[36px] items-center justify-center rounded-lg bg-[#34D399]">
                    {/* Icon here */}
                  </div>
                  <div className="w-full">
      ...
      {whyUsContent.map((content, index) => (
        <div key={index} className="rounded-sm border border-stroke bg-white mb-3 px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
          ...
          <form onSubmit={(e) => handleSubmitWhyUs(e, index, content._id)}>
            ...
            <div>
              <div className="flex justify-end">
               <button onClick={() => deleteWhyUsItem(content._id)}  className="rounded-sm bg-red-600 mr-3 p-2 px-3 text-sm font-medium text-gray hover:bg-opacity-90">Delete</button>
                <button
                  type="submit"
                  className="rounded-sm bg-orange-500 p-2 text-md font-medium text-gray hover:bg-opacity-90"
                >
                  Edit 
                </button>
              </div>
